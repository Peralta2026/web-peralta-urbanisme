        ...
        {ordered.map(member => {
          ...
          return (
            <div key={`mob-${member.slug}`}>
              ...
              <div
                onClick={() => toggle(member.slug)}
                style={{ position: "relative", width: "260px", maxWidth: "100%", aspectRatio: "260 / 344", overflow: "hidden", backgroundColor: "#c8c8c8", cursor: "pointer" }}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={`/team/${member.photo}`}
                ...
                <button
                  ...
                  aria-expanded={isActive}
                  aria-label={isActive ? "Tancar bio" : "Llegir bio"}
                  style={{ position: "absolute", bottom: "10px", right: "10px", width: "26px", height: "26px", border: "1px solid rgba(255,255,255,0.65)", borderRadius: "50%", background: "rgba(0,0,0,0.28)", color: "#fff", fontSize: "17px", fontWeight: 300, lineHeight: 1, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0, transition: "transform 0.35s cubic-bezier(0.22,1,0.36,1)", transform: isActive ? "rotate(45deg)" : "none" }}
                >
                  +
                </button>
              </div>
